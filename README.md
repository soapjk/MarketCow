...
行情读取会自动管理缓存：有效缓存直接返回；缓存缺失或过期时自动从上游刷新并
持久化；上游失败且仍有可接受的旧值时返回 `stale=true`、`cached=true` 的回退值。
完全没有可用数据时，单标的接口返回结构化 `503 unavailable`，批量接口则在
`errors` 中标注对应标的。批量刷新并发执行，调用方无需感知缓存命中状态。

`refresh=true` 仍可用于强制同步刷新：
