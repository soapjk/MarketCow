...
| 5 | Partial | UDS 0600 protocol; leased persistent job state/retry/cancel; persistent per-capability concurrency/minimum-interval dispatch policy and one-capability-per-process isolation; pre-promotion lease authorization; Rust-only hash/path/result-schema verification, promotion and transactional registration; full Python poll/start/complete/fail loop; deterministic SEC filing and bounded CSV inference handlers; Rust-owned optional worker pool with cleared credentials, bounded restart budgets, CPU/memory/FD/core limits and health/metrics | remaining provider handlers, provider-specific secret references and deletion of Python public API dependency |
