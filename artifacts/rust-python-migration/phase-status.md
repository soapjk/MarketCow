...
| 5 | Partial | UDS 0600 protocol; leased persistent job state/retry/cancel; pre-promotion lease authorization; Rust-only Artifact verification/promotion/registration; full Python poll/start/complete/fail loop; first deterministic SEC filing transform handler; Rust-owned optional worker pool with cleared credentials, bounded restart budgets, CPU/memory/FD/core limits and health/metrics | remaining provider handlers, per-provider rate limits/secret references and deletion of Python public API dependency |
