...
| 5 | Partial | UDS 0600 protocol; leased persistent job state/retry/cancel; pre-promotion lease authorization; Rust-only Artifact verification/promotion/registration; full Python poll/start/complete/fail loop; first deterministic SEC filing transform handler; Rust-owned optional worker pool with cleared credentials, bounded restart budgets and health/metrics | remaining provider handlers, resource limits and deletion of Python public API dependency |
