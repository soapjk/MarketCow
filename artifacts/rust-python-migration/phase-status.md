...
| 2 | Partial | health/readiness/scope/admin/metrics, auth/audit tests, fail-closed snapshot/events/checkpoint plus network WebSocket full-sync/resume/gap tests; bounded Rust MCP transport with native `service_health`, exact 14-tool loopback staged proxy, Python/Rust golden definitions and cross-process real-TCP differential call | remaining legacy HTTP route compatibility, native/worker-backed replacement of 13 MCP tools and removal of the Python public API proxy |
...
| 4 | Partial | workload ownership boundaries; fenced PostgreSQL job state; typed PostgreSQL Instrument repository with exact decimal semantics and atomic symbol mapping replacement; typed ClickHouse quote repository; disposable SQLite index; Rust content-addressed Artifact promotion and transactional job/manifest registration; PostgreSQL 17.10 and ClickHouse 25.8 real-instance tests for implemented repositories | wire Instrument repository into native public API/MCP; config/audit and remaining storage domains; broader Python/Rust differential tests; disable corresponding Python writers |
