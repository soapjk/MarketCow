...
| 4 | Partial | workload ownership boundaries; fenced PostgreSQL job state; typed ClickHouse quote repository; disposable SQLite index; Rust content-addressed Artifact promotion and transactional job/manifest registration; PostgreSQL 17.10 and ClickHouse 25.8 real-instance tests for implemented repositories | Instrument/config/audit and remaining storage domains; broader Python/Rust differential tests; disable corresponding Python writers |
| 5 | Partial | UDS 0600 protocol; leased persistent job state/retry/cancel; pre-promotion lease authorization; Rust-only hash/path/result-schema verification, promotion and transactional registration; full Python poll/start/complete/fail loop; deterministic SEC filing and bounded CSV inference handlers; Rust-owned optional worker pool with cleared credentials, bounded restart budgets, CPU/memory/FD/core limits and health/metrics | remaining provider handlers, per-provider rate limits/secret references and deletion of Python public API dependency |
