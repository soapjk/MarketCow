...
from marketcow.observability import (
    TELEMETRY_SCHEMA, Telemetry, record_operation, validate_snapshot,
)
...
class ObservabilityTest(unittest.TestCase):
    ...
    def test_versioned_schema_units_buckets_labels_and_no_duckdb(self):
        ...
        self.telemetry.histogram(
            "postgresql_query_latency_seconds", 0.025,
            operation="query", outcome="ok",
        )
        self.telemetry.histogram(
            "query_latency_seconds", 0.01, backend="clickhouse_canonical",
            query="range", outcome="ok",
        )
        snapshot = self.telemetry.snapshot()
        validate_snapshot(snapshot)
        self.assertEqual(snapshot["schema"], TELEMETRY_SCHEMA)
        rendered = json.dumps(snapshot, sort_keys=True)
        self.assertNotIn("duckdb", rendered)
        self.assertNotIn('"fallback"', rendered)
        pg = snapshot["metric_contract"]["postgresql_query_latency_seconds"]
    ...
    def test_all_telemetry_failures_are_fail_open(self):
        ...
        marker = []
        record_operation(Broken(), "record_operator", "audit", "ok")
    ...
    def test_authoritative_write_and_replay_emit_outcomes(self):
        class Repository:
        ...
        metric = next(item for item in self.telemetry.snapshot()["metrics"]
                      if item["name"] == "authoritative_write_total")
