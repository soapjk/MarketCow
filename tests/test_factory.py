...
from marketcow.factory import FactoryDependencies, create_online_repositories


class FactoryTest(unittest.TestCase):
    def settings(self, root: Path, *, scheduler: bool = True) -> Settings:
        return Settings( raw_path=root / "raw", profile="test",
            port=8793,
    ...
    def test_exact_routing_order_reverse_close_and_idempotence(self):
        with tempfile.TemporaryDirectory(suffix="-test") as folder:
            events = []
            resources = create_online_repositories(
    ...
    def test_preflight_precedes_every_constructor_or_file_side_effect(self):
        with tempfile.TemporaryDirectory(suffix="-test") as folder:
            events = []
            invalid = replace(self.settings(Path(folder)), allowed_root=None)
            with patch("pathlib.Path.open", side_effect=AssertionError("file opened")), \
                    self.assertRaisesRegex(ValueError, "explicit allowed root"):
                create_online_repositories(invalid, self.dependencies(events))
...
class FactoryIntegrationTest(unittest.TestCase):
    def test_real_postgres_clickhouse_factory_lifecycle(self):
        ...
        with tempfile.TemporaryDirectory(suffix="-test") as folder:
            root = Path(folder)
            settings = Settings( raw_path=root / "raw", profile="test", port=8793, postgres_dsn=dsn,
                postgres_schema="marketcow_test",
                ...
                allowed_root=root.parent, clickhouse_background_canonical=True,
            )
