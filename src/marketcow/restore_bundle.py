...
RESTORE_VERSION = "marketcow.local-restore.v1"
COMPONENT_ORDER = (
    "postgresql", "clickhouse", "artifact_archive", "authoritative_spool",
    "scheduler_state", "config_version", "migration_watermark", "cursor_key",
)
SUPPORTED = {
    ...
    "migration_watermark": {("logical-json", "marketcow.migration-watermark.v1")},
...
class LocalStorageRestore:
    ...
    def __init__(self, backup: LocalStorageBackup, targets: RestoreTargets,
        ...
        if targets.profile not in {"development", "test"}:
            raise ValueError("Storage  restore is development/test-only")
        ...
        self.state_root = self.root / ".storage-current-restore"
    ...
    def _preflight(self, artifacts: tuple[Path, ...]):
        ...
        if self.checkpoint_path.exists():
            ...
            checkpoint = json.loads(self.checkpoint_path.read_text())
            self._validate_checkpoint(checkpoint)
            allowed = {".storage-current-restore"}
