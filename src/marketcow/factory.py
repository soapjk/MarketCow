...
class OnlineRepositories:
    """Own the complete PostgreSQL/ClickHouse online resource graph."""
    ...
    def close(self) -> None:
        ...
        for resource in (
            ...
            try:
                ...
                errors.append(error)
        if errors:
            raise RuntimeError("resource shutdown failed") from errors[0]
    ...
    def health_snapshot(self) -> dict[str, Any]:
        ...
        try:
            ...
            pass
        return {"schema": HEALTH_SCHEMA, "components": {
    ...
    def __enter__(self) -> "OnlineRepositories":
        return self
...
def create_online_repositories(
    settings: Settings, dependencies: FactoryDependencies | None = None,
) -> OnlineRepositories:
    """Build the sole online PG/CH graph after a side-effect-free preflight."""
    settings.validate_preflight()
    deps = dependencies or FactoryDependencies()
    created: list[Any] = []
    try:
