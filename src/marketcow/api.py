...
from .polymarket_configured_scope import PolymarketConfiguredMarket, PolymarketConfiguredScope
...
def create_app(
    ...
    @app.get(
        ...
        summary="Read one atomic full-market discovery projection",
    )
    async def polymarket_discovery_full_sync():
        try:
            discovery = require_polymarket_discovery()
            return await run_polymarket_discovery(discovery.full_sync)
        except PolymarketLiveReadError as exc:
            _raise_polymarket_read_error(exc)
    ...
    async def polymarket_live_stream(
        ...
        try:
            scope = _require_polymarket_scope(market_id)
            async for message in polymarket_live_projection.stream_messages(scope, after_cursor):
