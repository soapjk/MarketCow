...
class RetentionDryRun:
    ...
    def dry_run(
        ...
        held = frozenset(str(item) for item in holds)
        candidates = []
        excluded = []
        for supplied in sorted({str(Path(item)) for item in artifacts}):
            ...
            try:
                ...
                excluded.append({
                    "artifact": supplied, "partition_id": None,
            ...
            candidates.append({
                "candidate_id": hashlib.sha256(
                    f"{self.policy.version}:{manifest['artifact_id']}".encode()
                ).hexdigest()[:24],
