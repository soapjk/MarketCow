...
from .backup_bundle import BackupComponent, LocalStorageBackup, _hash, _json
from .restore_bundle import LocalStorageRestore, RestoreTargets
MIGRATION_WATERMARK_VERSION = "marketcow.migration-watermark.v1"
from .postgres_migrations import POSTGRES_TRANSACTION_DOMAINS


BACKUP_MANIFEST_VERSION = "marketcow.pg-ch-backup-manifest.v1"
RESTORE_VERSION = "marketcow.pg-ch-restore.v1"
COMPONENT_ORDER = (
    "postgresql",
    "clickhouse",
    "artifact_archive",
...
REQUIRED_COMPONENTS = frozenset(COMPONENT_ORDER)
SUPPORTED = {
    "postgresql": {("logical-json", "postgres-16-current-18-domains")},
    "clickhouse": {("logical-json", "clickhouse-25.8-raw-canonical")},
    "artifact_archive": {("artifact-parquet-tree", "artifact-parquet-v1")},
    "authoritative_spool": {("wal-intent-tree", "authoritative-spool-v1")},
    "scheduler_state": {("scheduler-tree", "canonical-scheduler-v1")},
    "config_version": {("logical-json", "current-config-version-v1")},
    "migration_watermark": {("logical-json", MIGRATION_WATERMARK_VERSION)},
...
TREE_DESTINATIONS = {
    ...
    "config_version": ".marketcow/config",
    "migration_watermark": ".marketcow/migration",
}


def capture_postgresql(database: Any, captured_at: str) -> BackupComponent:
...
def capture_clickhouse(database: Any, captured_at: str) -> BackupComponent:
...
class LocalBackup(LocalStorageBackup):
    ...
    def _validate_components(components: Mapping[str, BackupComponent]) -> None:
        for name, expected in SUPPORTED.items():
            ...
            if (component.kind, component.version) not in expected:
                raise ValueError(f"unsupported  backup component {name}")
    ...
    def create(self, components: Iterable[BackupComponent], snapshot_at: str,
        ...
        if any(item["name"] == "duckdb" for item in result["components"]):
            raise ValueError("DuckDB is forbidden in a  online backup")
        return result

    def verify(self, artifact: Path) -> dict[str, Any]:
        manifest = super().verify(artifact)
        if tuple(item["name"] for item in sorted(
            manifest["components"], key=lambda item: COMPONENT_ORDER.index(item["name"])
        )) != COMPONENT_ORDER:
...
class LocalRestore(LocalStorageRestore):
    """Checkpointed empty-root restore for the pure PG/CH component graph."""

    def __init__(self, backup: LocalBackup, targets: RestoreTargets, clock=None) -> None:
        kwargs = {} if clock is None else {"clock": clock}
        super().__init__(
            backup, targets,
            component_order=COMPONENT_ORDER,
            supported=SUPPORTED,
            tree_destinations=TREE_DESTINATIONS,
    ...
    def record_verification(self, results: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if set(results) != required or any(value != "ok" for value in results.values()):
            raise ValueError(" restored-target verification is incomplete")
