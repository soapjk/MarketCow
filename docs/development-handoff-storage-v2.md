...
当前已验证基线：e8166d5（上一轮 Storage V2 本地完成状态）
当前本地候选：PostgreSQL + ClickHouse 独立蓝绿改造规划，待独立验收
...
### Provider-neutral 公共查询契约

公共 API 按业务能力命名，不再把供应商写入路由。简单读取保留 GET，复杂或批量查询使用 POST：
...
`provider` 可省略；省略时按市场、能力、健康状态和配置优先级自动选择。显式指定时默认
`allow_fallback=false`，不支持对应市场/能力的供应商返回
`provider_not_supported`，不得静默换源。响应使用统一业务 schema，并在 `source` 与
`routing` 中披露实际来源和请求策略。Provider 绑定的 `/v1/tushare/*` 仅作为旧客户端
deprecated 兼容入口；新调用不得依赖这些路由，后续主版本删除。
...
- **状态**：`已验收`（Artifact `a0ce718`）。新增 `storage-v2.telemetry.v1` process-local telemetry
...
- **状态**：`已验收`（Artifact `7d8c646`）。新增 `storage-v2.health.v1` 本地、process-local、并发安全的
...
- **状态**：`已验收`（Artifact `0a34202`）。新增 development-only `ParquetColdArchive`，构造时强制
...
- **状态**：`已验收`（Artifact `134c032`）。新增版本化 `storage-v2.retention-policy.v1` 与纯只读
...
- **状态**：`已验收`（Artifact `1ce0fe5`）。新增 `storage-v2.production-readiness.v1` 本地准备包。
  ...
  checks 且全部通过，所有固定 operation 必须带目标端独立回读 row/checksum；容量公式会从 measured raw
  rows/bytes、model rows 与 source count 重新推导 bytes/row、modeled online bytes、含 30% reserve 的
  ...
  前序证据不再接受最小状态 JSON：021A 重新验证 manifest 内部 checksum、content-addressed backup id、六组件
  集合及逐文件大小/checksum；021B 要求六项 restored-target verification、完整组件集合、backup chain 与 watermark；
  022A/022B 分别绑定带内部 checksum 的 durable checkpoint，并核对全 16 个 PostgreSQL 域、raw/canonical/
  contract、稳定 completion fingerprint，以及 switched→DuckDB rollback event；023 使用真实 disposable
  ClickHouse benchmark 报告。测试支持通过显式 `MARKETCOW_READINESS_EVIDENCE_ROOT` 将这些真实本地 drill
  Artifact 导出到 gitignored development 根，普通测试不产生仓库副作用。
  ...
  stage checker 还返回具体 checked facts：configuration 从 `Settings` 静态核对默认 DuckDB/ClickHouse-disabled、
  credential reference 与 8790 逻辑目标；schema 将当前源码 PG/CH migration max version 与 package 逐项比较；
  backup 绑定 verified backup id 和 restore chain；backfill 核对 19 个数据域、lag=0 与 durable checkpoint；
  read switch 必须看到 rolled_back/DuckDB 及 rollback event；observation 重新核对 14 checks、11 operations 与
  容量推导结果。
...
- **状态**：`已验收`（Artifact `f529164` + 文档收口 `b1eee69`）。新增
  `storage-v2.final-acceptance.v1` 本地最终门禁，固定绑定
  SV2-012～SV2-024 的 18 个已验收 commit，并逐一验证 commit 存在且为当前 HEAD 祖先、交接文档对应
  条目标为已验收且包含 Artifact、SV2-EXT 仍为需用户授权。门禁重新校验已验收 readiness
  manifest/package/runbook checksum、SV2-024 release binding，以及七类 external action 全部
  unauthorized/unexecuted。

  最终门禁只在 development/test 隔离根写入原子 report；按固定命令矩阵复跑默认套件、contract gate、
  PostgreSQL、ClickHouse、真实恢复/回填/读切换/benchmark 联合演练、Ruff 和 diff-check。每项记录有界
  duration、return code 和脱敏输出 hash，任一非零即 fail-closed，不生成 passed 结论。最终报告明确只证明
  synthetic/disposable 本地 Storage V2 完成，production 与远程动作仍属于 SV2-EXT，未获授权且未执行。
...
## 十一、PostgreSQL + ClickHouse 独立蓝绿改造冻结清单

### 11.1 规划基线与目标边界

本清单于 `e8166d5` 基线上冻结。只完成 Storage V2 的本地 synthetic/disposable 蓝绿架构改造，
不等同于 MarketCow 所有未来产品功能完成。2026-07-21 盘点时，`127.0.0.1:8791` 返回
`profile=development`、`metadata_backend=duckdb`、`storage_health.status=disabled`，数据库仍为
`storage://warehouse/market_data.duckdb`；`.env.development` 使用 T9 worktree 下的
`data-development`，ClickHouse 未启用。代码中 `MarketCowService` 仍会构造 `Warehouse`，
`create_stage1_repositories` 仍组装 DuckDB primary、ClickHouse shadow 及 DuckDB fallback。

冻结后的目标在线架构为：

```text
V2 API / Service
  ├── PostgreSQL：全部事务数据、元数据、基本面、控制面、Artifact manifest
  ├── ClickHouse：全部 raw/canonical 行情写入与读取
  └── 本地文件：有界 WAL/spool、原始 Artifact、已验证 Parquet 冷归档

旧正式服务（独立蓝环境）── DuckDB，保持不变，作为整体回滚目标
离线迁移工具 ──────────── DuckDB 旧数据副本只读输入
```

V2 在线启动、API、Repository、readiness、health 和 fallback 链不得实例化、打开或查询 DuckDB。
DuckDB 只允许出现在显式离线导入器、用户另行授权后取得的旧数据副本、test fixture 或分析工具；
这些入口必须与在线模块、配置和依赖注入物理隔离。默认开发验证只使用合成数据、T9 隔离根以及本机
disposable PostgreSQL/ClickHouse，不读取真实 production 数据。

本清单正常情况下不得重排、复用或静默改写 ID。验收发现阻断缺陷时，新增父项关联的
`BG-xxx-REV-n` 或 `BG-xxx-BLOCK-n`，记录证据、阻断原因、验收标准和解决提交；非阻断新需求进入
Backlog，不影响 `BG-020` 完成判定，除非用户明确修改整体目标。每轮只实施一个 ID。

### 11.2 固定执行顺序

`BG-001 → BG-002 → BG-003 → BG-004 → BG-005 → BG-006 → BG-007 → BG-008 → BG-009 → BG-010 → BG-011 → BG-012 → BG-013 → BG-014 → BG-015 → BG-016 → BG-017 → BG-018 → BG-019 → BG-020`

`BG-EXT` 仅在 `BG-020` 后由用户对确切动作另行授权，不属于本地完成定义。

### `BG-001`：蓝绿 ADR 与模块依赖禁区

- **目标**：用新 ADR 固化独立蓝绿决策、数据库职责和 DuckDB 在线禁区。
- **范围**：更新 ADR-002 后继决策；建立 online/offline import 模块边界和禁止依赖图。
- **依赖**：本规划 Artifact。
...
- **目标**：定义 V2 必须同时具备 PostgreSQL 与 ClickHouse 的 fail-closed 启动配置。
- **范围**：新增明确 V2 profile/backend schema；隔离 root、loopback/test 目标、超时、凭证引用和禁止 DuckDB 配置。
- **依赖**：`BG-001`。
...
- **目标**：使 PostgreSQL 覆盖全部事务性、元数据、基本面、控制面与 Artifact manifest 契约。
- **范围**：盘点并补齐 migration、约束、PIT/history/upsert、JSONB 和所有 Repository 方法。
- **依赖**：`BG-002`。
...
- **目标**：让 ClickHouse 独立实现全部 raw/canonical 行情读写，不依赖 DuckDB adapter。
- **范围**：recent/range/page/exact/matrix/as-of/raw、quotes 最新值、版本收敛、选源与 provenance。
- **依赖**：`BG-002`。
...
- **目标**：把影子写 writer 改为 ClickHouse 在线权威主写及 durable fail-closed/fail-aware 原语。
- **范围**：微批、稳定幂等 ID、原子 WAL、配额、重放、dead-letter、canonical intent 和 operator 接口。
- **依赖**：`BG-004`。
...
- **目标**：基于 ClickHouse raw 独立完成 canonical 构建、重放衔接与生命周期。
- **范围**：确定性选源、质量状态、版本、精确范围 rebuild、durable queue、lease、pause/resume/shutdown。
- **依赖**：`BG-005`。
...
- **目标**：建立 V2 在线唯一装配入口并移除 DuckDB primary/shadow/fallback。
- **范围**：PostgreSQL metadata/fundamentals/artifacts + ClickHouse market bars + spool/scheduler/telemetry 生命周期。
- **依赖**：`BG-003`、`BG-006`。
...
- **目标**：使 V2 Service/API 仅依赖 Repository 工厂。
- **范围**：移除 `warehouse` 在线创建和隐式 SQL/路径假设；刷新、缓存、Artifact 写入与 admin 路由经 PG/CH。
- **依赖**：`BG-007`。
...
- **目标**：health/readiness 只反映 PostgreSQL、ClickHouse、WAL/canonical queue，不暴露或探测 DuckDB。
- **范围**：四态、滞回、原因、数据库逻辑标识、依赖组合和敏感信息脱敏。
- **依赖**：`BG-008`。
...
- **目标**：建立不读取 production 数据的旧版 API 路由与响应契约基线。
- **范围**：从 main 代码/合成 fixture 捕获路由、方法、默认参数、状态码、字段类型、错误 schema 和 snapshot。
- **依赖**：`BG-001`。
...
- **状态**：`已验收`（Artifacts `ebf1da0`、`f9a46e2`、`9a5656a`、`b1acd3c`）。以旧 main `701ffbd` 的隔离 development app 与确定性 fake
  ...
  每条公开 method/path 均具备唯一双侧执行状态与逐类 applicability/不适用原因；190 个逐路由场景分别以双侧
  ...
  `backend_failure` 必须为可解析且带 `detail/error/code/message` 明确错误字段的 5xx，non-JSON 500 与无错误字段的
  普通 JSON 500 均拒绝。quotes 的 200 soft batch errors 独立归类为 `partial_failure` 并冻结 old/V2 `errors`
  shape，不能冒充 HTTP backend failure；不能满足者按具体 route/side 具名 `not_applicable`，不得伪称 executed。
...
- **目标**：逐端点决定严格兼容、显式版本化或有记录的不兼容。
- **范围**：修复/版本化 refresh 默认、批量错误结构、health.database；覆盖所有旧路由、参数和错误。
- **依赖**：`BG-009`、`BG-010`。
...
- **目标**：将 DuckDB 限定为显式只读旧副本输入和测试/分析工具。
- **范围**：独立 CLI/package、source fingerprint、schema/version、只读打开、路径 containment、进程入口隔离。
- **依赖**：`BG-003`、`BG-004`。
...
- **目标**：在 disposable 环境演练全域全量迁移。
- **范围**：PostgreSQL 全事务域、ClickHouse raw/canonical、Artifact 引用；keyset batch、checkpoint、checksum。
- **依赖**：`BG-011`、`BG-012`。
...
- **目标**：定义旧服务继续运行时，副本增量进入 V2 的有界 high-watermark/catch-up 流程。
- **范围**：稳定 source watermark、增量日志/manifest、checkpoint、重复消费、lag 和完成闸门。
- **依赖**：`BG-013`。
...
- **目标**：准备可在未来授权后验证真实旧库副本的只读、脱敏、有界流程。
- **范围**：复制输入 manifest 规范、抽样/全域检查、敏感信息、容量上限、报告和授权前检查。
- **依赖**：`BG-014`。
...
- **目标**：证明纯 PG/CH V2 可从本地可校验备份恢复，不要求 DuckDB 在线组件。
- **范围**：PG、CH、Artifact/Parquet、WAL/intents、cursor key、watermark、空环境恢复与 canonical rebuild。
- **依赖**：`BG-014`。
...
- **目标**：覆盖直接写入、查询、WAL、canonical、PG、CH、缓存及 operator 的本地 telemetry/SLO。
- **范围**：有界指标、结构化日志、health 原因、spool CLI、dead-letter、审计和恢复操作。
- **依赖**：`BG-009`、`BG-016`。
...
- **目标**：以真实写入和目标回读证明 V2 容量、延迟、深分页、恢复和资源边界。
- **范围**：warm/cold、并发、raw/canonical 写、PG 事务、spool replay、backup/restore、磁盘模型和 soak。
- **依赖**：`BG-011`、`BG-016`、`BG-017`。
...
- **目标**：在两个独立 disposable 服务之间演练消费者目标切换和整体回滚，不使用进程内 fallback。
- **范围**：旧 DuckDB 合成服务保持蓝；V2 PG/CH 为绿；preflight、观察窗口、stop condition、审计和回滚。
- **依赖**：`BG-015`、`BG-018`。
...
- **目标**：由验收方判定独立 PG/CH Storage V2 本地范围是否完成。
- **范围**：核对 BG Artifact 链、在线 DuckDB 禁区、全套测试、迁移/恢复/蓝绿演练、性能、安全、文档和限制。
- **依赖**：`BG-001`～`BG-019` 及登记的 REV/BLOCK 全部验收。
...
- **目标**：仅在本地完成后，按用户逐项授权执行真实副本复制、production 准备或发布。
- **范围**：可能包括确切 production 数据副本复制、PG/CH 正式连接/迁移、上游切换、整体回滚、launchd、push、PR、部署、上传或发布。
- **依赖**：`BG-020` 和用户对每个目标、数据、可见性、保留后果、动作及回滚的后续明确确认。
- **验收标准**：只执行明确批准的动作；未批准项保持未执行；每步有备份、观察和回滚证据。
- **必要测试**：由届时授权范围决定。
- **排除项**：任何未逐项授权的外部动作。
- **状态**：`可选、未授权、未开始`。
...
只有 `BG-001`～`BG-020` 及正式登记的阻断修订全部由验收方独立接受，且最终门禁证明 V2 在线
启动、写入、查询、fallback、health/readiness 均不实例化或访问 DuckDB，验收方才能判定
“独立 PostgreSQL + ClickHouse Storage V2 本地 synthetic/disposable 范围完成”。这不授权
`BG-EXT`，也不代表所有 MarketCow 未来产品功能完成。真实 production 数据复制、连接、迁移、消费者
切换、launchd 修改、push、PR、部署、上传和发布始终需要用户随后对确切动作单独授权。

## 十二、当前运行检查结果

最近已验证基线为 `b1eee69`（SV2-025）。SV2-012～SV2-025 均已由验收方逐项接受；冻结清单定义的
Storage V2 本地 synthetic/disposable 开发范围已经完成。最终验收报告位于
`data-development/sv2-025-final-development/storage-v2-final-acceptance.json`，SHA-256 为
`572845df7ceaab7671652312ef998ae40b0ec4f8a25176f302b8f796a3cbd51d`，绑定 HEAD `b1eee69` 与
18 个已验收 Artifact。最近一次完整默认套件为 271 项通过、30 项显式集成跳过；最终固定矩阵中的
contract gate、PostgreSQL、ClickHouse、真实恢复/回填/读切换/benchmark 联合演练、Ruff 与
diff-check 均通过。

新一轮独立蓝绿改造以 `e8166d5` 为基线，当前仅完成规划盘点，功能编码冻结。盘点时 development
服务监听 `127.0.0.1:8791`，health 显示 `profile=development`、`metadata_backend=duckdb`、
`storage_health.status=disabled`，ClickHouse 未启用；未为盘点停止、重启或修改该服务。第十一节
`BG-001`～`BG-020` 是新的固定本地执行清单，上一轮 SV2 完成状态只是本次重构的输入基线，不代表
新蓝绿目标已经完成。

本地完成不等于执行 SV2-EXT。production 连接、迁移、切换、
launchd 更新、push、PR、部署、上传和发布仍全部需要用户对确切动作另行授权。
...
步骤。先以第十一节 BG 固定清单核对唯一下一项，只执行验收方指定的一个清单项，完成后暂停；
保持现有 API 契约和全部测试通过，不要连接或迁移正式 PostgreSQL/ClickHouse，也不要
读取真实 production 数据或远程推送。除非验收确认必须插入阻断修复，不得跳项、并行推进或
自行改写清单顺序；不得在 V2 在线路径重新引入 DuckDB primary、shadow 或 fallback。
```
