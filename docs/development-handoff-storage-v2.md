...
本节是从当前状态到“本地可交付完成”的唯一有序清单，取代此前按六个宽泛步骤描述的
待办路线。除验收发现必须先插入的阻断修复外，后续开发必须按 ID 顺序逐项下发、实现、
验证和验收；每次只推进一项。状态定义如下：
...
- **状态**：`已实现待验收`；初始候选 `b975460` 的两项缺陷已在本地候选 `51b0c2b` 修复，
  不能在独立验收前标记完成。
...
### `SV2-014`：横截面分页与多时间点查询边界

- **目标**：完成全市场横截面的可扩展读取能力，不让大 symbol 集合依赖单次 5000 行响应。
- **范围**：先实现精确单时间点按 symbol keyset 分页；再提供显式、有界的多时间点矩形查询，
  使用稳定 `(bar_time, symbol)` 顺序并定义最大时间点、symbol 和返回行数。
- **前置依赖**：`SV2-013`。
- **验收标准**：精确时间不混入陈旧值；多时间点边界明确；跨页无丢失重复；DuckDB、
  ClickHouse canonical 与 fallback 等价；API 参数错误在查询前返回 400。
- **必要测试**：symbol/时间边界、整页/尾页/空页、过滤绑定、版本 `FINAL`、故障回退、
  大样本有界性、默认与真实 ClickHouse 回归。
- **排除项**：最近有效 bar（非精确）、后台物化、production 切换。
- **状态**：`未开始`。
...
### `SV2-019`：可观测性、健康与本地 SLO

- **目标**：提供足以判断写入、重放、canonical、查询和 fallback 健康度的稳定指标与诊断。
- **范围**：有界 counters/gauges/histograms、structured logs、readiness/degraded 状态；至少覆盖
  ingest latency、pending/failed/replayed、canonical lag、mismatch、query latency/fallback、
  cache freshness、merge/disk pressure；敏感错误需截断和脱敏。
- **前置依赖**：`SV2-018`。
- **验收标准**：指标基数有界；健康接口不泄露凭证/绝对敏感路径；阈值和 SLO 文档化；disabled
  backend 不误报；诊断不能改变读写语义。
- **必要测试**：固定时钟、错误脱敏/长度、label cardinality、状态转换、并发快照、故障注入、
  API schema 回归。
- **排除项**：外部监控 SaaS、告警消息发送、production dashboard 部署。
- **状态**：`未开始`。

### `SV2-020`：冷热分层与 Parquet 归档原语

- **目标**：验证 ADR-002 的在线/冷存储边界，并保持冷数据可校验、可查询、可恢复。
- **范围**：在隔离开发数据上定义 retention policy；按 market/interval/source/year/month 原子导出
  Parquet；manifest、checksum、schema/version、watermark；DuckDB 离线查询与回填接口。
- **前置依赖**：`SV2-019`。
- **验收标准**：归档前后行数/键/校验和一致；重复导出幂等；部分失败可恢复；只有成功校验后才
  允许形成删除候选，且本项不实际删除在线或 production 数据。
- **必要测试**：月界/时区/schema evolution、重复导出、崩溃恢复、损坏文件、DuckDB round-trip、
  代表性压缩率与查询验证。
- **排除项**：production TTL/DELETE、云对象存储上传、正式数据清理。
- **状态**：`未开始`。

### `SV2-021`：备份与恢复

- **目标**：建立 PostgreSQL、ClickHouse、DuckDB 兼容数据、Parquet/artifact、spool 与 cursor key
  的一致备份/恢复手册和本地工具。
- **范围**：明确 RPO/RTO 假设、备份清单/校验和/版本、密钥权限、恢复顺序和 canonical 可重建边界；
  仅使用一次性本地实例与合成数据演练。
- **前置依赖**：`SV2-020`。
- **验收标准**：空环境可恢复；恢复后 migration、行数、业务键、PIT、查询契约、cursor key 与
  artifact 引用一致；故意损坏可被检测；文档包含失败与回退步骤。
- **必要测试**：全量与增量样例、跨版本 migration、丢单组件、密钥缺失、checksum 失败、恢复后
  默认/PG/CH 契约套件。
- **排除项**：备份或上传真实 production 数据、远程备份目的地、production restore。
- **状态**：`未开始`。

### `SV2-022`：迁移、回填与回滚演练

- **目标**：用可丢弃本地数据完整演练 DuckDB→PostgreSQL/ClickHouse 的迁移、增量追平、对账、
  读开关切换和回滚。
- **范围**：版本化 runbook、preflight、checkpoint/watermark、可恢复 backfill、双写追平、验收闸门、
  dry-run 和 rollback；所有命令默认指向 development/test 且拒绝 production 标识。
- **前置依赖**：`SV2-021`。
- **验收标准**：重复迁移幂等；中断续跑无丢失重复；全域对账通过；切换后查询契约一致；回滚后
  DuckDB 可用且新写数据处置明确；失败注入均有恢复路径。
- **必要测试**：多阶段中断、乱序/重复输入、schema upgrade、lag 追平、切换/回滚、备份恢复组合、
  完整合成数据演练报告。
- **排除项**：连接、读取、写入或迁移真实 production 数据；正式开关变更。
- **状态**：`未开始`。
...
收口对应 `SV2-012`～`SV2-019`；冷热分层及交付准备对应 `SV2-020`～`SV2-025`。正式执行
只属于 `SV2-EXT`。
