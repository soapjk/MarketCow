...
- **状态**：`本地实现完成，待独立验收`。新增 development/test-only `LocalStorageBackfill`，仅接受显式
  ...
  ingested_at 与相同版本 content-rank 的确定性收敛规则；源文件指纹在一轮前后相同才进入 reconcile，最多
  10 轮（可显式限制到 1..100），否则以 lag 未归零失败。全域闸门逐表比较行数、稳定业务键顺序和规范化
  内容 checksum；JSON、UTC、Float 表示先规范化；raw 行数与 canonical 业务键基数分别用 ClickHouse FINAL
  核对，并可接入已验收 Storage V2 contract gate。任何非白名单 mismatch 阻止 complete，报告只保留逻辑
  标识、watermark、批次/pass、lag=0、域状态与有界恢复说明，不含 DSN、凭证或绝对路径。

  6 项默认单元测试覆盖隔离拒绝、checkpoint 篡改、JSON/Float 规范化、provenance、跨重启 canonical 完整
  范围与脱敏报告。一次性 PostgreSQL 16 + ClickHouse 25.8 联合演练在 PostgreSQL batch 后注入中断，再于
  恢复前追加 DuckDB-primary 行情：重启后 snapshot 与 catch-up 收敛，raw/canonical 各 2 行、lag=0、重复运行
  run ID 稳定且无逻辑重复。当前默认套件 243 项通过（27 项显式集成跳过）；独立 PostgreSQL 8 项、
