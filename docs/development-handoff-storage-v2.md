...
- **状态**：`本地实现完成，待独立验收`。新增 development/test-only
  ...
  多项或结果 checksum 不稳定均 fail-closed。

  每个 operation 统一记录三轮以上 p50/p95/p99、最差 rows/s、逻辑 checksum、物理/逻辑 bytes 和实际
  EXPLAIN；page SQL 单独保留并拒绝 `OFFSET`，deep page 还必须包含 `bar_time > cursor` keyset 条件。
  报告同时记录有界硬件/组件版本、进程 peak memory/thread、ClickHouse free ratio 与 merge backlog；内容
  统一敏感扫描后以临时文件 fsync、rename、目录 fsync 原子发布。
  ...
  **预先固定的本地 benchmark SLO**：raw write ≥1,000 rows/s；canonical rebuild ≥500 rows/s；warm/cold、
  ...
  默认测试使用受控时钟覆盖 percentile、SLO、容量、隔离、结果不稳定、OFFSET 和不足 30%/merge backlog
  负向闸门。显式集成以一个完整合成月（20 trading days × 240 minute bars × 2 sources，共 9,600 raw
  rows）连接一次性 ClickHouse 25.8，并使用真实 Warehouse、ReliableClickHouseWriter、canonical builder、
  keyset query/EXPLAIN、ParquetColdArchive、spool outage/replay 和四线程独立 ClickHouse sessions；三轮
  warm/cold、写入、rebuild、归档/恢复与故障恢复报告全部满足上述 SLO。
