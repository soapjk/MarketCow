...
- **状态**：`本地复核通过，待独立验收`。新增共享一致性模块
  ...
  **允许差异只有两类**：其一，`backend`、`attempted_backend`、`fallback`、`error` 是有界路由
  diagnostics，不属于数据响应；其二，旧 recent/range 与 raw 查询的 `source_payload` 在 DuckDB
  中保留任意 provider 原始 JSON，而 ClickHouse schema 只保留结构化 provenance，无法无损重建
  provider 私有字段。该字段只在上述旧/raw 契约的跨后端 gate 中显式排除；`source`、
  `observed_at`、`ingested_at`、`raw_artifact_id`、OHLCVA 及 canonical page 的结构化 provenance
  仍必须逐字段相同。除此之外 mismatch 非零立即使 gate 失败。当前本地验证：156 项默认发现
  （22 项显式外部集成跳过）通过；共享 gate 4 项通过；一次性 ClickHouse 25.8 上完整 17 项
