...
- **状态**：`本地实现完成，待独立验收`。新增 `storage-v2.final-acceptance.v1` 本地最终门禁，固定绑定
...
最近已验证基线为 `1ce0fe5`（SV2-024）。SV2-012～SV2-024 均已由验收方逐项接受；
最近一次完整默认套件为 269 项通过、30 项显式集成跳过，真实 disposable 恢复/回填/读切换/
benchmark 联合演练 4 项通过，ClickHouse+contract 24 项、PostgreSQL 8 项通过。SV2-024 的
真实本地 evidence ledger、readiness package 与 rehearsal 位于 gitignored `data-development`
隔离根，六个 package-bound stage dry-run 均返回 checked facts，且未连接 production。

当前只执行 SV2-025 最终本地验收；本地完成不等于执行 SV2-EXT。production 连接、迁移、切换、
