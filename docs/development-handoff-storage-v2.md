...
- **状态**：`本地实现完成，待独立验收`。新增 `storage-v2.telemetry.v1` process-local telemetry
  ...
  全 schema 最大 series 数小于 500。ClickHouse disabled 时不组装 telemetry backend、不连接、不建目录、
  不启线程，独立 disabled snapshot 只显示 enabled=false 且不伪造 pressure series。

  ClickHouse development factory 只在 backend 已显式启用后创建共享 telemetry，并接入 writer/WAL、
  scheduler/canonical、所有 canonical/raw 查询与 fallback、API cache freshness、contract gate 及显式
  ClickHouse pressure 采样入口；Shadow diagnostics 提供本地 snapshot。query wrapper 保留原返回与异常，
  telemetry 失败 fail-open。结构化日志 event/severity 也是固定枚举，字段/列表数量有界；统一将凭证、
  token/API key、DSN URL 与绝对敏感路径替换为 REDACTED，并将所有文本限制为 1,000 字符。

  当前本地验证：6 项 telemetry 固定时钟/schema/桶/单位/cardinality/并发/脱敏/故障测试通过；
  ...
  通过；Ruff 与 diff-check 通过。范围未进入 `SV2-019B`，未实现 readiness/degraded 状态机、SLO、
  Prometheus/OTel、外部告警、部署或远程写入。
