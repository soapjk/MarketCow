...
- **状态**：`本地实现完成，待独立验收`。新增 development-only `LocalStorageBackup` 与显式
  ...
  authenticated stream sealing，备份文件权限固定 0600；其他文件固定 0640，内容扫描拒绝 DSN、password、
  token/API key/authorization。wrapping key 不写入 manifest/bundle。
  ...
  6 项本地测试覆盖 full/incremental、重复幂等、跨组件 watermark、manifest/file 损坏、缺组件、cursor
  权限与明文负向扫描、source/artifact symlink、敏感 DSN、publish 崩溃清理、8 路并发互斥及唯一 rename。
