...
- **状态**：`本地实现完成，待独立验收`。新增 development/test-only `LocalReadSwitchDrill`，以
  ...
  本项 8 项默认单元测试与 1 项显式真实 ClickHouse 演练通过；完整默认套件 258 项通过，其中 28 项
  外部存储 opt-in 测试因默认未配置而跳过；真实 ClickHouse 回归连同本演练 20 项、PostgreSQL 回归
  8 项、Storage V2 contract gate 5 项、Ruff 与 diff-check 均通过。测试容器均为本地 disposable 实例。
