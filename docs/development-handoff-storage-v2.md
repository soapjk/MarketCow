...
- **状态**：`本地实现完成，待独立验收`。新增 development/test-only `LocalStorageRestore`，只接受名称与
  ...
  原表/列恢复。ClickHouse DateTime/UInt 类型在逻辑 JSON 回填时规范化。cold 与 spool 保持 bundle 相对路径、
  ...
  full/incremental 顺序、未来版本、cursor 旧 token、报告脱敏；另有真实一次性 PostgreSQL 16 + ClickHouse
  25.8 联合空环境恢复，验证 migration 后业务行、raw FINAL、DuckDB/cold/spool 和 cursor key。
