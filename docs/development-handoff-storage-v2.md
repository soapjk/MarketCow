...
- **状态**：`本地实现完成，待独立验收`。新增稳定的本地命令入口
  `marketcow --profile development spool <action>`，提供机器可读 JSON 与明确退出状态；支持有界
  `status`、`list`、`audit`、`replay`、`quarantine-corrupt`、`retry-dead` 和
  ...
  当前本地验证：37 项 operator/writer/scheduler/config 目标测试通过；176 项默认发现（23 项 opt-in
