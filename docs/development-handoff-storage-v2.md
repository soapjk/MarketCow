...
- canonical 单 symbol 闭区间支持显式 keyset 分页：同时提供 start/end/page_size（1..5000）
  ...
  配置；secret 轮换会使旧游标失效。DuckDB 与 ClickHouse 均使用 `bar_time > after`
  keyset 条件而非 OFFSET；ClickHouse 失败以同一 after/边界回退 DuckDB。
...
- 已完成 canonical 单 symbol 闭区间的版本化、完整性保护、查询绑定 keyset 游标分页，
  DuckDB/ClickHouse/fallback 页内容与 next_cursor 等价；旧无游标范围响应保持兼容；
- 尚未完成 raw、横截面分页或更大规模的生产容量基准；
...
```text
feature/storage-v2 检查点 11 已验证基线：ef86ad1；检查点 12 候选变更只扩展 canonical
单 symbol 闭区间 keyset 游标分页
默认测试：120 项通过；16 项显式外部存储集成测试因未配置本地服务而跳过
...
DuckDB fallback 等价契约。检查点 12 候选已完成 canonical 单 symbol 闭区间的 HMAC
keyset 游标、无 OFFSET 的 DuckDB/ClickHouse 查询及同游标 fallback；10,001 行本地样本
逐页验证有界、无缺失重复。尚未完成 raw/横截面分页、生产容量基准、非精确或多时间点
