...
mutation. Read-only does not mean every implementation is cache-only. `get_quotes` explicitly
forces `refresh=false`; some dividend/fundamental reads may refresh MarketCow's internal cache
through their public read contract. Batch sizes and history page sizes are bounded to protect
the agent context window. For reproducible analysis, prefer `get_canonical_bars`, follow
`next_cursor`, and retain the returned manifest, provenance, quality, and adjustment fields.
