...
`stream_id`. A client must verify consecutive event sequences. On reconnect it supplies
both the prior `resume_stream_id` and `resume_after`. The server replays retained matching
events before new events. It returns `gap_unrecoverable` if the stream ID changed, the
...
source. Quote/trade financial values remain decimal strings.
...
`unsubscribe`, `subscription_ack`, `stream_heartbeat`, and `stream_error`. Market events
