...
pg_root=$(mktemp -d /tmp/marketcow-control-plane-pg.XXXXXX)
pg_port=${MARKETCOW_EPHEMERAL_CONTROL_PLANE_POSTGRES_PORT:-55434}
pg_started=false
...
MARKETCOW_TEST_POSTGRES_DSN="postgresql://marketcow_test@127.0.0.1:$pg_port/marketcow_test" \
  ...
  postgres_control_plane_round_trip_when_test_dsn_is_configured \
  -- --ignored --nocapture
...
jq -n \
  --arg schema_version "marketcow.control-plane-postgres-integration.v1" \
  ...
  '{schema_version:$schema_version,git_commit:$git_commit,passed:true,
    postgres:{version:$postgres_version,
      test:"postgres_control_plane_round_trip_when_test_dsn_is_configured",
      passed:true,ephemeral_root:$pg_root,
      assertions:["safe_forward_migration_idempotent",
