...
pg_root=$(mktemp -d /tmp/marketcow-audit-pg.XXXXXX)
pg_port=${MARKETCOW_EPHEMERAL_AUDIT_POSTGRES_PORT:-55435}
pg_started=false
...
MARKETCOW_TEST_POSTGRES_DSN="postgresql://marketcow_test@127.0.0.1:$pg_port/marketcow_test" \
  ...
  postgres_audit_repository_is_append_only_when_test_dsn_is_configured \
  -- --ignored --nocapture
...
jq -n \
  --arg schema_version "marketcow.audit-postgres-integration.v1" \
  ...
  '{schema_version:$schema_version,git_commit:$git_commit,passed:true,
    postgres:{version:$postgres_version,
      test:"postgres_audit_repository_is_append_only_when_test_dsn_is_configured",
      passed:true,ephemeral_root:$pg_root,
      assertions:["safe_forward_migration_idempotent","schema_v1_round_trip",
        "unicode_and_decimal_string_preserved","exact_duplicate_idempotent",
        "conflicting_duplicate_rejected","action_and_outcome_filter",
