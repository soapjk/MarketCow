...
def main():
    ...
    old = runtime / 'releases/bounded-v2'
    release = runtime / 'releases/bounded-v2-decode-r1'
    ...
    updated = original.replace(str(old), str(release)).replace('/logs/bounded-v2-', '/logs/bounded-v2-decode-r1-')
    ...
    with (runtime / 'logs/bounded-v2-decode-r1-activation.json').open('x') as file:
