...
test ! -e "$runtime/logs/public-api-build-r6.exit-code"
finish() { status=$?; printf '%s\n' "$status" > "$runtime/logs/public-api-build-r6.exit-code"; }
trap finish EXIT
cd /mnt/p44pro/projects/marketcow-shadow-v3
touch crates/marketcowd/src/source_publication.rs crates/marketcowd/src/source_public_api.rs crates/marketcowd/src/source_discovery_api.rs
cargo test --offline --locked --release -j4 --bin marketcow-discovery-collector
